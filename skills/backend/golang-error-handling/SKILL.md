...
description: >-
  Diretrizes e padrões idiomáticos para tratamento de erros em Golang no Nexus Commerce — criação de erros com contexto, wrapping com %w, erros sentinela, errors.Is/As, regra de single handling (logar OU retornar, nunca ambos), structured logging com slog e boas práticas de observabilidade.
---
...
- **Coding mode** — writing new error handling code. Follow the best practices sequentially; check for swallowed errors, missing wrapping context, and log-and-return pairs.
- **Review mode** — reviewing error handling changes. Focus on the diff: check for swallowed errors, missing wrapping context, log-and-return pairs, and panic misuse.
- **Audit mode** — auditing existing error handling across a codebase targeting independent categories (creation, wrapping, single-handling rule, panic/recover, structured logging).
...
When auditing error handling across a large codebase, focus on these 5 independent error categories:

- Category 1: Error creation — validate `errors.New`/`fmt.Errorf` usage, low-cardinality messages, custom types
- Category 2: Error wrapping — audit `%w` vs `%v`, verify `errors.Is`/`errors.As` patterns
- Category 3: Single handling rule — find log-and-return violations, swallowed errors, discarded errors (`_`)
- Category 4: Panic/recover — audit `panic` usage, verify recovery at goroutine boundaries
- Category 5: Structured logging — verify `slog` usage at error sites, check for PII in error messages
